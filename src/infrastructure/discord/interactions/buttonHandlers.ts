...
export async function handleButtonInteraction(
  ...
  // Solo el Sensei puede usar botones de gestión
  if (mission.senseiId !== interaction.user.id) {
    await interaction.reply({ content: '❌ Solo el Sensei de esta expedición puede gestionarla.', ephemeral: true });
    return;
  }
  ...
  try {
    ...
    if (action === 'eliminar') {
      ...
      await interaction.reply({ content: `🗑️ Expediente **${mission.code}** eliminado.`, ephemeral: true });
      return;
    }

    await interaction.reply({ content: '⏳ Acción en desarrollo.', ephemeral: true });
  } catch (error: any) {
